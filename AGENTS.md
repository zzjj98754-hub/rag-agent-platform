...
- **`runGenerationEvaluation()`**: RAGAS-style Faithfulness / Answer Relevance / Context Relevance — keyword-overlap approximation (mock LLM). **Note**: scores are relative rankings, not absolute quality measures. Switch to LLM-as-Judge (GPT-4/Codex) for production.
