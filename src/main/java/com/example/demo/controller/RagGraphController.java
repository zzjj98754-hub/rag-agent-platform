...
public class RagGraphController {
    ...
    @GetMapping("/runs/{id}") public RagGraphState get(@PathVariable String id) { var u=user.requireCurrentUser(); return official.map(o -> graphState(o.state(id,u.id(),u.role()), id)).orElseGet(() -> graph.get(id,u.id(),u.role())); }
